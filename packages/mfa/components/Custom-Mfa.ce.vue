<template>
  <div :class="!isCips ? 'comp-new' : 'comp'">
    ...
    <template v-else>
      ...
      <div :class="!isCips ? 'subhead-new' : 'subhead'" v-if="!isLogin">
      ...
      <hr v-if="isEditing && !isLogin" :class="!isCips && 'hr-new'" />
      <Transition name="slide-up" appear>
...
// set store, handle props
const loading = ref(true);
store.isCips = props.appType === 'cips';
store.isLogin = props.fromMfaLogin === 'true';
const { isCips, isLogin } = store;
const isHint = props.fromMfaHint === 'true';
// resolve images url's & calculate logo ratio
...
// get status
onMounted(async () => {
  !isLogin && (await getMfaStatus());
  isEditing.value = isHint;
...
const getMfaStatus = async () => {
  const received = await useFetch(
    resolveBaseUrl(action(ACTIONS.CHECK_STATUS)),
    METHOD,
...
const mfaGenerateQrCode = async (mfaMandatory = false) => {
  const received = await useFetch(
    resolveBaseUrl(action(ACTIONS.GENERATE_QR_CODE)),
    METHOD,
    payload(ACTIONS.GENERATE_QR_CODE),
    { isLogin: mfaMandatory }
  );
  if (!received.error) {
...
const mfaActivate = async () => {
  const received = await useFetch(
    resolveBaseUrl(action(ACTIONS.ACTIVATE)) +
...
const mfaDeactivate = async () => {
  const received = await useFetch(
    resolveBaseUrl(action(ACTIONS.DEACTIVATE)),
    METHOD,
...
const mfaCheckVerificationCode = async () => {
  ...
  const received = await useFetch(
    resolveBaseUrl(action(ACTIONS.CHECK_VERIFICATION_CODE)) + path,
    METHOD,
...
const mfaDownloadBackupCodes = async () => {
  const received = await useFetch(
    resolveBaseUrl(action(ACTIONS.DOWNLOAD_BACKUP_CODES)),
    METHOD,
...
const mfaGenerateNewBackupCodes = async () => {
  const received = await useFetch(
    resolveBaseUrl(action(ACTIONS.GENERATE_NEW_BACKUP_CODES)),
    METHOD,
...
const mfaCancel = () => {
  if (isLogin)
    window.location =
      resolveBaseUrl(action(ACTIONS.CANCEL), true) +
      '?action=' +
      ACTIONS.CANCEL;
  isEditing.value = false;
};
